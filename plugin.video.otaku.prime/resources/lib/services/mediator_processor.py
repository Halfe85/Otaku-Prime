...
def _merge_placements(anilist,mal):
    ...
    mal_rows=mal.get("episodes") or []; merged=[]
    for index,row in enumerate(result.get("episodes") or []):
        merged.append(_merge_dict(row,mal_rows[index] if index<len(mal_rows) else {}))
...
class MediatorProcessor:
    ...
    def resolve(self,item):
        ...
        if anilist and mal:
            try:
                combined=_merge_placements(anilist,mal); combined["provider_attempts"]=attempts; return combined
            except MediatorProviderConflict as exc:
                attempts.append({"provider":"anilist+mal","skipped":False,"error":str(exc)})
                LOGGER.warning("AniList/MAL structural disagreement for Prime item %s: %s",item.get("local_id"),exc)
        elif anilist:
