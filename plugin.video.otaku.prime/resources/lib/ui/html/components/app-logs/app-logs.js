(function () {
  ...
  if (!toggle || !windowElement) return;
  ...
  function poll() {
    fetch("/api/logs?after="+lastId,{headers:{"Accept":"application/json"}}).then(function(response){
      ...
      (payload.entries||[]).forEach(append);state.textContent="Live updates connected";
      if (payload.entries&&payload.entries.length) messages.scrollTop=messages.scrollHeight;
