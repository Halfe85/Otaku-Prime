(function () {
  ...
  function franchiseOf(entry){return entry.franchise_english_name||entry.franchise_romaji_name||"";}
  ...
  function placementOf(entry){
    if(entry.placement_resolved){var value=(entry.metadata_provider||"metadata")+" · S"+String(entry.metadata_season_number==null?0:entry.metadata_season_number).padStart(2,"0");if(entry.metadata_episode_number!=null)value+="E"+String(entry.metadata_episode_number).padStart(2,"0");return value;}
    return entry.relation_resolved?"Awaiting metadata":"Awaiting franchise";
  }
  function render(){
    var term=search.value.trim().toLowerCase(),wanted=status.value;
    var visible=entries.filter(function(entry){var haystack=[titleOf(entry),entry.romaji_name,entry.native_name,franchiseOf(entry),entry.provider,entry.provider_item_id].join(" ").toLowerCase();return(!term||haystack.indexOf(term)!==-1)&&(!wanted||entry.list_status===wanted);});
    ...
    if(!pageEntries.length){var empty=document.createElement("tr");textCell(empty,"No matching watchlist entries.","muted").colSpan=7;rows.appendChild(empty);}
    pageEntries.forEach(function(entry){
      ...
      textCell(row,entry.media_format||"Unknown");textCell(row,entry.list_status);textCell(row,String(entry.progress||0)+(entry.episode_count!=null?" / "+entry.episode_count:""));textCell(row,franchiseOf(entry)||"Awaiting resolution");textCell(row,placementOf(entry),entry.placement_resolved?"placement-ready":"placement-pending");rows.appendChild(row);
    });
