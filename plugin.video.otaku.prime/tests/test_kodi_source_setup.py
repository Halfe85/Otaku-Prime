...
class KodiSourceSetupTests(unittest.TestCase):
    def test_adds_tv_source_and_preserves_existing_source(self):
        with tempfile.TemporaryDirectory() as profile:
            with open(os.path.join(profile,"sources.xml"),"w",encoding="utf-8") as handle:
                handle.write("<sources><video><source><name>Existing</name>"
                             "<path>/media/existing/</path></source>"
                             "<source><name>Otaku Prime Movies</name>"
                             "<path>/old/movies/</path></source></video></sources>")
            library=StreamLibraryService(os.path.join(profile,"library")); library.initialize()
            setup=KodiSourceSetupService(profile,library)
            result=setup.ensure_sources()
            self.assertEqual(1,len(result["added"]))
            self.assertEqual(["Otaku Prime Movies"],result["removed"])
            ...
            self.assertEqual(["Existing","Otaku Prime TV Shows"],names)
